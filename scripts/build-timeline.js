...
var SEED = {
  ...
  people: { type: 'person', level: 2, list: [
    ...
    'A. C. N. Nambiar', 'Suhasini Chattopadhyay'
  ]},
...
var MANUAL_SPANS = {
  ...
  'A. C. N. Nambiar': [1896, 1986], 'Suhasini Chattopadhyay': [1902, 1973],
...
var PERSON_DESCS = {
  ...
  'A. C. N. Nambiar': 'Indian nationalist and colleague of Subhas Chandra Bose',
  'Suhasini Chattopadhyay': 'first woman Communist of India and sister of Sarojini Naidu',
...
var TOPIC_DESCS = {
  ...
  // UPSC regulators, commissions and schemes (upscCommissions / upscSchemes seeds)
  'GST Council': 'constitutional body that decides indirect-tax rates and reforms, chaired by the Union Finance Minister',
  'NITI Aayog': 'government think-tank that replaced the Planning Commission to shape national development policy',
  'Competition Commission of India': 'statutory regulator that prevents practices harming competition and enforces the Competition Act',
  'Securities and Exchange Board of India': 'statutory regulator of the Indian securities market, IPO issuance and investor protection',
  'Telecom Regulatory Authority of India': 'statutory regulator of telecommunications pricing, quality and standards',
  'Insurance Regulatory and Development Authority of India': 'statutory regulator licensing and supervising insurance products and insurers',
  'National Bank for Financing Infrastructure and Development': 'development finance institution that funds long-term infrastructure projects',
  'National Green Tribunal': 'specialised environmental court that expedites environment and forest disputes',
  'Insolvency and Bankruptcy Board of India': 'regulatory body that runs the insolvency resolution process under the IBC',
  'Financial Stability and Development Council': 'apex inter-regulatory body that monitors and strengthens financial stability',
  'Jal Jeevan Mission': 'flagship scheme to provide safe tap water to every rural household',
  'PM-KISAN': 'income-support scheme transferring direct cash to all farmer households',
  'Pradhan Mantri Awas Yojana': 'Housing for All scheme for affordable houses, urban and rural',
  'Smart Cities Mission': 'urban renewal programme that builds citizen-centric smart cities',
  'Atal Mission for Rejuvenation and Urban Transformation': 'urban transformation scheme improving basic infrastructure in mission cities',
  'National Rural Livelihood Mission': 'scheme empowering rural poor through self-help groups and livelihoods',
  'Pradhan Mantri Grameen Sadak Yojana': 'scheme connecting habitations with all-weather roads',
  'Direct Benefit Transfer': 'system that credits subsidies and benefits directly to bank accounts via Aadhaar linkage',
  'Aspirational Districts Programme': 'programme accelerating development in India\u2019s poorest districts',
  'Pradhan Mantri Jan Arogya Yojana': 'Ayushman Bharat flagship health cover of up to Rs 5 lakh per family per year',
  'Minimum Alternate Tax': 'minimum corporate tax payable when book profits exceed normal taxable income',
  'Securities Transaction Tax': 'tax levied on the value of securities transactions on stock exchanges',
  'Equalisation Levy': 'tax on digital/online services provided by non-resident e-commerce operators',
  'Insolvency and Bankruptcy Code': '2016 law consolidating insolvency resolution into a single time-bound process',
  'Digital Rupee': 'Reserve Bank of India\u2019s central bank digital currency in token form',
  'Unified Payments Interface': 'instant inter-bank payment system that transfers money by mobile number or UPI ID',
  'Monetary Policy Committee': 'RBI body that sets the policy interest rate to meet the inflation target',
  'National Investigation Agency': 'central agency probing terrorism and other scheduled offences across states',
  'Central Bureau of Investigation': 'India\u2019s premier central investigating police agency for corruption and major crimes',
  'District Mineral Foundation Trust': 'statutory trust that spends mining royalties on welfare of mining-affected districts',
  'National Cooperative Development Corporation': 'statutory body financing cooperatives and their economic development',
  'Export\u2013Import Bank of India': 'apex development finance institution financing the country\u2019s foreign trade',
  'Small Industries Development Bank of India': 'apex development bank financing micro, small and medium enterprises'
};
...
var PERSON_ROLE = /\b(?:monk|nun|scholar|philosopher|poet|writer|author|novelist|playwright|painter|artist|sculptor|musician|singer|vocalist|composer|dancer|actor|actress|filmmaker|director|producer|athlete|cricketer|footballer|boxer|wrestler|shooter|archer|sprinter|runner|hockey player|badminton|chess grandmaster|grandmaster|tennis|javelin|swimmer|gymnast|king|queen|emperor|empress|prince|princess|ruler|monarch|sultan|raja|maharaja|nawab|politician|statesman|president|minister|governor|parliamentarian|senator|diplomat|ambassador|jurist|judge|lawyer|advocate|reformer|activist|revolutionary|leader|saint|guru|mystic|prophet|founder|pioneer|scientist|physicist|chemist|biologist|astronomer|mathematician|botanist|zoologist|geologist|engineer|physician|doctor|surgeon|psychologist|economist|sociologist|anthropologist|archaeologist|historian|linguist|philologist|orientalist|missionary|philanthropist|industrialist|businessman|entrepreneur|banker|educator|academic|professor|journalist|editor|photographer|explorer|navigator|conqueror|warrior|general|admiral|commander|soldier|officer|astronaut|pilot|inventor|administrator|bureaucrat|agronomist|planter|craftsman|priest|bishop|cardinal|pope|caliph|imam|chancellor|viceroy|governor-general|politician|spiritualist|theologian|reformer|economist|activist|bacteriologist|biochemist|biophysicist|geophysicist|geographer|meteorologist|climatologist|oceanographer|ecologist|seismologist|volcanologist|paleontologist|palaeontologist|paleobotanist|ethnologist|entomologist|ichthyologist|ornithologist|mycologist|herpetologist|lepidopterist|malacologist|parasitologist|taxonomist|horticulturist|veterinarian|pharmacist|nurse|midwife|dentist|ophthalmologist|cardiologist|neurologist|pathologist|psychiatrist|epidemiologist|criminologist|demographer|statistician|actuary|cartographer|lexicographer|translator|interpreter|polyglot|librarian|curator|archivist|calligrapher|engraver|printmaker|caricaturist|cartoonist|illustrator|choreographer|ballerina|comedian|acrobat|puppeteer|rapper|lyricist|librettist|screenwriter|conductor|instrumentalist|guitarist|pianist|violinist|cellist|trumpeter|drummer|organist|tenor|soprano|baritone|narrator|satirist|humorist|essayist|biographer|memoirist|diarist|columnist|critic|reporter|broadcaster|commentator|orator|storyteller|fabulist|tsar|czar|kaiser|shah|pharaoh|zamindar|archduke|elector|margrave|viscount|baron|earl|duchess|marquess|baronet|regent|dictator|despot|tyrant|warlord|chieftain|shaman|astrologer|alchemist|architect|surveyor|aviator|cosmonaut|mountaineer|alpinist|climber|skier|skater|cyclist|racer|driver|jockey|sailor|mariner|paratrooper|commando|sniper|sergeant|corporal|colonel|brigadier|marshal|commodore|trooper|cavalryman|watchmaker|jeweller|blacksmith|weaver|potter|carpenter|mason|baker|brewer|vintner|winemaker|chef|tailor|couturier|dressmaker|milliner|shoemaker|printer|publisher|abbess|anchorite|hermit|pilgrim|mayor|sheriff|magistrate|consul|prefect|satrap|legate|tribune|centurion|praetor|commissar|mercenary|pirate|privateer|conquistador|mobster|gangster|gang leader|smuggler|spymaster)\b/i;
...
function personDescFor(name, qs) {
  ...
  var reB = /(?:The|A|An)\s*_{3,}\s+(?:was|is)\s+([A-Za-z][^()?]{2,90}?)\s*\(\s*(?:c\.?\s*)?(-?\d{3,4})\s*[-\u2013]\s*(-?\d{3,4})\s*\)/gi;
...
// ---------------------------------------------------------------------------
// Kin-entity promotion: auto-provision a person node for everyone who appears ONLY
// in a kinship relation ("Jochi was the son of Börte, the first wife of the Mongol
// leader Temüjin" / "...later reconquered by Samprati, Kunala's son") but is
// otherwise invisible to the lineage miner (no own question topic, no descriptor).
// Runs once over the whole corpus before relation extraction, so the resolver and
// the PERSON_KIN gate can fire edges between minor relatives too. Also promotes an
// existing non-person node bound in a real kin relation (Jochi ships today as a
// 'concept'); every affected node is tagged kin:true so extractRelations' resolver
// keeps it regardless of the prominence cap.
// ---------------------------------------------------------------------------
var KIN_REGION_NAMES = ['india', 'bharat', 'hindustan', 'china', 'japan', 'korea', 'england', 'britain', 'scotland', 'wales', 'ireland', 'france', 'germany', 'italy', 'spain', 'portugal', 'netherlands', 'belgium', 'austria', 'hungary', 'poland', 'russia', 'ukraine', 'turkey', 'greece', 'egypt', 'iran', 'iraq', 'afghanistan', 'pakistan', 'bangladesh', 'sri lanka', 'nepal', 'bhutan', 'myanmar', 'burma', 'thailand', 'vietnam', 'cambodia', 'laos', 'malaysia', 'indonesia', 'philippines', 'singapore', 'australia', 'new zealand', 'usa', 'america', 'canada', 'mexico', 'brazil', 'argentina', 'chile', 'peru', 'colombia', 'south africa', 'nigeria', 'kenya', 'ethiopia', 'europe', 'asia', 'africa', 'eurasia', 'middle east', 'central asia', 'southeast asia', 'south asia', 'east asia', 'west asia', 'arabia', 'persia', 'mesopotamia', 'anatolia', 'scandinavia', 'iberia', 'balkans', 'caucasus', 'siberia', 'tibet', 'kashmir', 'punjab', 'bengal', 'bihar', 'rajasthan', 'gujarat', 'maharashtra', 'tamil nadu', 'kerala', 'karnataka', 'odisha', 'assam', 'delhi', 'mumbai', 'chennai', 'kolkata', 'hyderabad', 'rome', 'byzantium', 'constantinople'];
var KIN_NON_PERSON_END = /\b(?:dynast(?:y|ies)|empire|kingdom|republic|states?|provinces?|districts?|count(?:y|ies)|cities?|towns?|villages?|rivers?|mountains?|ranges?|deserts?|seas?|oceans?|lakes?|bays?|straits?|gulfs?|islands?|archipelago|peninsulas?|valleys?|plateaus?|regions?|countr(?:y|ies)|nations?|subcontinents?|continents?|hemispheres?|coasts?|borders?|capitals?|arm(?:ies|y)?|nav(?:ies|y)?|actors?|actresses?|pharaohs?|militias?|brigades?|regiments?|air\s*forces?|forces?|troops?|police|railways?|railroads?|departments?|ministries?|commissions?|committees?|councils?|boards?|bureaus?|agencies?|authorities?|corporations?|companies?|firms?|groups?|syndicates?|banks?|funds?|exchanges?|markets?|institutes?|universities?|colleges?|schools?|academies?|hospitals?|temples?|churches?|mosques?|gurudwaras?|monasteries?|ashrams?|forts?|palaces?|monuments?|memorials?|museums?|libraries?|stadiums?|parks?|bridges?|dams?|canals?|highways?|expressways?|airports?|ports?|ships?|submarines?|aircraft|missiles?|rockets?|satellites?|programmes?|programs?|schemes?|projects?|policies?|plans?|initiatives?|campaigns?|movements?|missions?|alliances?|coalitions?|blocs?|pacts?|treaties?|accords?|amendments?|sections?|clauses?|articles?|wars?|battles?|sieges?|conflicts?|uprisings?|revolts?|rebellions?|revolutions?|coups?|massacres?|pogroms?|invasions?|expeditions?|crusades?|festivals?|eras?|ages|periods?|centur(?:y|ies)|decades?|millennia|doctrines?|theories?|concepts?|principles?|ideologies?|religions?|faiths?|sects?|cultures?|civilizations?|civilisations?|planets?|galax(?:y|ies)|constellations?|asteroids?|comets?|nobels?|prizes?|awards?|medals?|cups?|trophies?|games?|olympics|dynastic|princely|chieftains?|warriors?|soldiers?|wom[ae]n|boys?|girls?|followers?|supporters?|members?|officials?|princes?|regents?|januar(?:y|ies)|februar(?:y|ies)|mar[ct]h|april|mays?|june|jul(?:y|ies)|augusts?|septembers?|octobers?|novembers?|decembers?|mondays?|tuesdays?|wednesdays?|thursdays?|fridays?|saturdays?|sundays?|bc|bce|ad|ce|indeed|meanwhile|thenceforth)\b/i;
var KIN_TITLE_STRIP = ['the', 'a', 'an', 'king', 'queen', 'emperor', 'empress', 'maharaja', 'maharani', 'raja', 'rani', 'sultan', 'sultana', 'nawab', 'begum', 'prince', 'princess', 'shah', 'sha', 'tsar', 'czar', 'tsarina', 'khan', 'sheikh', 'mirza', 'malik', 'rao', 'saheb', 'bahadur', 'haji', 'hajji', 'mullah', 'imam', 'pir', 'guru', 'sant', 'saint', 'lady', 'lord', 'sir', 'dame', 'dr', 'doctor', 'prof', 'professor', 'colonel', 'major', 'captain', 'general', 'lieutenant', 'marshal', 'admiral', 'commander', 'minister', 'president', 'premier', 'prime', 'pharaoh', 'ruler', 'leader', 'monarch', 'sovereign', 'head', 'chief', 'chieftain', 'governor', 'viceroy', 'diwan', 'dewan', 'pandit', 'chhatrapati', 'sardar', 'bishop', 'archbishop', 'pope', 'cardinal', 'abbot', 'monk', 'priest', 'rabbi', 'caliph', 'amir', 'emir', 'wazir', 'sayyid', 'aga', 'agha', 'mir'];
var KIN_ETHNIC = ['mongol', 'mongolian', 'turkic', 'turkish', 'hunnic', 'scythian', 'parthian', 'kushan', 'hittite', 'phrygian', 'lydian', 'median', 'assyrian', 'babylonian', 'sumerian', 'sassanian', 'seleucid', 'hellenic', 'hellenistic', 'macedonian', 'epirote', 'illyrian', 'thracian', 'arvanite', 'arab', 'arabian', 'persian', 'egyptian', 'nubian', 'ethiopian', 'axumite', 'phoenician', 'carthaginian', 'berber', 'magyar', 'avars', 'bulgar', 'viking', 'norse', 'celtic', 'gaulish', 'saxon', 'frankish', 'angle', 'slavic', 'vandal', 'gothic', 'ostrogothic', 'visigothic', 'lombard', 'albanian', 'illyric', 'indian', 'mauryan', 'gupta', 'chola', 'pandya', 'chera', 'rashtrakuta', 'pratihara', 'pala', 'vakataka', 'chinese', 'japanese', 'korean', 'vietnamese', 'thai', 'burmese', 'khmer', 'tibetan', 'afghan', 'nepali', 'ghaznavid', 'seljuk', 'ghurid', 'timurid', 'mamluk', 'ottoman', 'safavid', 'abbasid', 'umayyad', 'fatimid', 'ayyubid', 'ilkhanid'];
function kinTrimName(nm) {
  var toks = String(nm || '').split(/\s+/).filter(Boolean);
  while (toks.length > 1 && (KIN_ETHNIC.indexOf(toks[0].toLowerCase()) !== -1 || KIN_TITLE_STRIP.indexOf(toks[0].toLowerCase()) !== -1)) toks.shift();
  while (toks.length > 1 && (KIN_TITLE_STRIP.indexOf(toks[toks.length - 1].toLowerCase()) !== -1 || KIN_ETHNIC.indexOf(toks[toks.length - 1].toLowerCase()) !== -1)) toks.pop();
  var out = toks.join(' ').trim();
  if (/^([A-Z]\.\s*)+$/.test(out)) return out;
...
var KIN_RAW_SMALL = '[A-Z][A-Za-z\u00C0-\u017F\u2019\'.\-]+(?:\\s+[A-Z][A-Za-z\u00C0-\u017F\u2019\'.\-]+)?';
var KIN_OBJ = '[A-Z][A-Za-z\u00C0-\u017F\u2019\'.\-]+(?:(?:\\s+[a-z]+)*\\s+[A-Z][A-Za-z\u00C0-\u017F\u2019\'.\-]+)*';
var KIN_NUM = 'first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|only|youngest|eldest';
var KIN_WORD = '(?:great(?:[- ]*great){0,2}[- ]+)?(?:father|mother|son|daughter|brother|sister|grandfather|grandmother|grandson|granddaughter|grandparent|grandchild|grandchildren|uncle|aunt|nephew|niece|cousin|sibling|child|children|parent|parents|spouse|wife|husband|widow|widower|consort|heir|heiress|fiance|fiancee|bride|groom|descendant|ancestor|forefather)(?:s|es)?(?:[- ]in[- ]law)?|(?:father|mother|son|daughter|brother|sister|child|parent|spouse)[- ]in[- ]law|(?:half|step)[- ](?:brother|sister|father|mother|son|daughter|child|parent|sibling)';
function kinCanoniceName(nm) {
  var n = String(nm || '').replace(/\s+/g, ' ').trim();
  n = n.replace(/^[\(\[\{"']+|[,.;:!?\)\]\}'"]+$/g, '').trim();
  n = n.replace(/\s+of\s+(?:the\s+)?/g, ' ').trim();
  if (/^[A-Z]\.([A-Z]\.)*$/.test(n)) return '';
  var toks = n.split(/\s+/);
  while (toks.length > 1 && KIN_TITLE_STRIP.indexOf(toks[0].toLowerCase()) !== -1) toks.shift();
  return toks.join(' ').trim();
}
function kinPersonOk(nm) {
  var c = kinCanoniceName(nm);
  if (!c || c.length < 3) return false;
  if (/^\d{3,4}s?$/.test(c)) return false;
  if (/[&+]|(?:\s+|^)(?:and|or)\s+/.test(c)) return false;
  ...
  // that the raw name regex picks up when the kin verb follows a pronoun or
  // conjunctive fragment. Titles (King, Sir, …) are already stripped in
  // kinCanoniceName, so a surviving role word means the token is not a name.
  if (KIN_ROLE_WORDS.indexOf(c.split(' ')[0].toLowerCase()) !== -1) return false;
  // Reject whole-clause fragments where a raw name class swallowed most of a
  // sentence ("Peter III died of the plague while commanding the Aragonese army
  // at Mayorga"). Real personal names embed free-standing lowercase words only
  // as name particles (van, von, de, ibn, al, ...), never verbs/nouns.
  var toks = c.split(' ');
  for (var i = 1; i < toks.length - 1; i++) {
    if (/^[a-z]+(?:\.[a-z]+)?$/.test(toks[i]) && !KIN_PARTICLES[toks[i]]) return false;
  }
  // Reject role-word mixtures ("…Emperor Joseph II Emperor Leopold II Emperor
  // Francis…") where a title token survives inside the name instead of leading
  // it; a surviving title mid-name means the capture spilled past a title into
  // the next phrase (real names keep titles only at the front, already stripped).
  // Skip name particles like "the"/"of" ("Alexander the Great") which also
  // appear in KIN_TITLE_STRIP as leading articles.
  for (var j = 1; j < toks.length; j++) {
    if (KIN_TITLE_STRIP.indexOf(toks[j].toLowerCase()) !== -1 && !KIN_PARTICLES[toks[j].toLowerCase()]) return false;
  }
  // Reject sentence-break artifacts where the dot-in-name class jumps a full
  // stop into the next sentence ("Ashoka. He"), while keeping initials "J. R. D.".
  if (/[A-Za-z]{2,}\.\s+/.test(c) || /[a-z]\.(?:\s|$)/.test(c)) return false;
  // Reject answer-echo duplicates from the joined fact+answer rows ("Ashoka Ashoka").
  if (/(\S+)\s+\1\b/i.test(c)) return false;
  // Reject possessive / clause residue ("Ashoka's", "Kavad's second eldest son
  // Jamasp", "Midwest Television founder August Meyer") picked up by the raw
  // name classes, then a trailing full stop on a real name ("Vikramajit.") or a
  // single-uppercase sentence join ("Attila.In") — while keeping initials
  // ("J. R. D.", "J.R.D. Tata") which carry their own interior dots.
  var raw = String(nm || '');
  if (/['\u2019]s\b/.test(c) || /['\u2019]s\b/.test(raw)) return false;
  if (/\b(?:the|a|an)\s+[a-z]+\s+[A-Z]/.test(raw)) return false;
  // Trailing full stop on a real name ("Vikramajit.") and sentence-join dots
  // ("Attila.In") were already cleaned off the canonical form, so check the RAW
  // name: allow initials ("J. R. D.", "J.R.D. Tata") but nothing else that ends
  // in punctuation or joins a lowercase run to a capital through a dot.
  if (/[.,;:!?]+$/.test(raw) && !/^([A-Z]\.\s*)+$/.test(raw)) return false;
  if (/^[A-Z][a-z]+\./.test(raw)) return false;
  return true;
}
function kinNameToSeg(name, txt, at) {
  return txt.slice(Math.max(0, at - 200), Math.min(txt.length, at + 260));
}
function kinSpanFor(seg) {
  var b = bioSpan(seg);
  if (b) return b;
  var y = yearSignals(seg);
  if (!y) return null;
  var pts = Object.keys(y.trusted).map(Number);
...
function provisionKinPersons(eachQuestion, nodes, topicMap) {
  ...
  // 200k silently truncated mid-corpus, so mine past that with headroom.
  var KIN_SEEK_BUDGET = 2000000;
  var lookup = {};
  var peopleByLastToken = {};
  for (var qn of nodes) {
    var qk = canonName(qn.name);
    if (qk) lookup[qk] = qn;
    for (var qa of (qn.aliases || [])) { var qk2 = canonName(qa); if (qk2 && !lookup[qk2]) lookup[qk2] = qn; }
    if (qn.type === 'person' || qn.seed) {
      var qtk = qk ? qk.split(' ').pop() : '';
      if (qtk) (peopleByLastToken[qtk] = peopleByLastToken[qtk] || []).push(qn);
    }
  }
  var prov = {};
  var stats = { provisioned: 0, promoted: 0, seen: 0 };
  var reOf = new RegExp('(' + KIN_RAW_FULL + ')\\s+(?:was|is|became|had)\\s+(?:a|an|the|his|her|their)\\s+(?:(?:' + KIN_NUM + ')\\s+)?(' + KIN_WORD + ')\\s+of\\s+(' + KIN_RAW_SMALL + ')', 'g')
  var reOfApp = new RegExp('(' + KIN_RAW_SMALL + ')\\s*,\\s*(?:the|a|an)?\\s*(?:(?:' + KIN_NUM + ')\\s+)?(' + KIN_WORD + ')\\s+of\\s+(?:[a-z]+\\s+)?(' + KIN_OBJ + ')', 'g');
  var rePoss = new RegExp('(' + KIN_RAW_FULL + ')[\\u2019\\x27]s\\s+(?:(?:' + KIN_NUM + ')\\s+)?(' + KIN_WORD + ')\\s*,\\s*(' + KIN_RAW_SMALL + ')', 'g')
  var rePossInv = new RegExp('(' + KIN_RAW_SMALL + ')\\s*,\\s*(' + KIN_RAW_FULL + ')[\\u2019\\x27]s\\s+(?:[a-z]+\\s+)?(' + KIN_WORD + ')', 'g')
  var reSpouse = new RegExp('(' + KIN_RAW_FULL + ')\\s+(?:had\\s+)?(?:married|wed)\\s+(' + KIN_RAW_FULL + ')', 'g')
  var reSpouseP = new RegExp('\\b(?:he|she)\\s+(?:had\\s+)?(?:married|wed)\\s+(' + KIN_RAW_FULL + ')', 'g')
  var reMarriage = new RegExp('(' + KIN_RAW_FULL + ')[\\u2019\\x27]s\\s+marriage\\s+to\\s+(' + KIN_RAW_FULL + ')', 'g')
  var reKids = new RegExp('(' + KIN_RAW_FULL + ')[\\u2019\\x27]s\\s+(?:and\\s+[a-z]+\\s+)?(?:child|son|daughter|children|sons|daughters)\\s*[:,\\u2014]\\s*((?:' + KIN_RAW_SMALL + ')(?:\\s*,\\s*(?:and\\s+)?' + KIN_RAW_SMALL + '){0,3})', 'g')
  function note(name, seg) {
    if (stats.seen >= KIN_SEEK_BUDGET) return;
    stats.seen++;
    name = kinTrimName(name);
    ...
    if (matchLong) {
      matchLong.aliases = (matchLong.aliases || []).concat([name]);
      if (!topicMap[canon]) topicMap[canon] = matchLong.id;
      matchLong.kin = true;
      return;
    }
    prov[key] = true;
    var node = {
      ...
      timebase: span ? null : 'undated',
      level: 2,
      cats: [],
      count: 1,
      kin: true,
      aliases: [],
      desc: cleanDesc(seg) || 'person'
    };
    nodes.push(node);
    topicMap[canon] = node.id;
    stats.provisioned++;
  }
  function kinScanQuestion(it) {
    var q = it.q;
    ...
    var m;
    reOf.lastIndex = 0;
    while ((m = reOf.exec(txt))) {
      var seg = kinNameToSeg('', txt, m.index);
      if (m[1]) note(m[1], seg);
      if (m[3]) note(m[3], seg);
    }
    reOfApp.lastIndex = 0;
    while ((m = reOfApp.exec(txt))) { var segA = kinNameToSeg('', txt, m.index); if (m[1]) note(m[1], segA); if (m[3]) note(m[3], segA); }
    rePoss.lastIndex = 0;
    while ((m = rePoss.exec(txt))) { var segP = kinNameToSeg('', txt, m.index); if (m[1]) note(m[1], segP); }
    rePossInv.lastIndex = 0;
    while ((m = rePossInv.exec(txt))) { var segQ = kinNameToSeg('', txt, m.index); if (m[1]) note(m[1], segQ); if (m[2]) note(m[2], segQ); }
    reSpouse.lastIndex = 0;
    while ((m = reSpouse.exec(txt))) { var segS = kinNameToSeg('', txt, m.index); if (m[1]) note(m[1], segS); if (m[2]) note(m[2], segS); }
    reSpouseP.lastIndex = 0;
    while ((m = reSpouseP.exec(txt))) { var segT = kinNameToSeg('', txt, m.index); if (m[1]) note(m[1], segT); }
    reMarriage.lastIndex = 0;
    while ((m = reMarriage.exec(txt))) { var segU = kinNameToSeg('', txt, m.index); if (m[1]) note(m[1], segU); if (m[2]) note(m[2], segU); }
    reKids.lastIndex = 0;
    while ((m = reKids.exec(txt))) { var segV = kinNameToSeg('', txt, m.index); if (m[1]) note(m[1], segV); if (m[2]) m[2].split(',').forEach(function (kn) { note(kn.trim(), segV); }); }
  }
  eachQuestion(kinScanQuestion);
  return stats;
}
...
function extractRelations(scanSource, nodes, topicMap) {
  ...
  // Kin-promoted persons (auto-provisioned lineage endpoints) must be resolvable
  // even though their mention count places them outside the prominence cap.
  for (var kn of nodes) { if (kn.kin === true) relationNodes.push(kn); }
  ...
  var JUNK_KIN_ENDPOINTS = ['generally', 'native', 'around', 'according', 'american', 'four', 'run', 'brown', 'father', 'child', 'william', 'sam', 'apapolo', 'basch', 'aus', 'indiana', 'apocryphon', 'hebrew bible', 'jews', 'main', 'daughter', 'find', 'british', 'internet', 'place', 'law', 'guide', 'six', 'big', 'mother', 'son', 'three', 'seven', 'former', 'met', 'older', 'eventually', 'however', 'secondly', 'subsequently', 'previously', 'instead', 'soon', 'jewish', 'male', 'next', 'man', 'london', 'new york'];
  ...
  // A longer "of" gap is safe ONLY when it is a run of lowercase epithet words
  // ("brother of prominent political activist and poet Sarojini Naidu"): no other
  // name, no nested kin/of chain, no clause starter — the next mention is still
  // the direct referent. Anything else keeps the tight 24-char rule.
  function isEpithetGap(g) {
    if (g.length > 64) return false;
    if (/[A-Z0-9\u2019']/.test(g)) return false;
    var words = g.toLowerCase().match(/[a-z]+/g) || [];
    ...
    for (var i = 0; i < words.length; i++) {
      ...
      if (w === 'of' || KIN_REL[w] || /\b(?:who|which|that|whose|when|while|because|as|along|with|including|where|named|called|known)\b/.test(w)) return false;
    }
    return true;
  }
  ...
  // Boundaries that end a kin-LIST walk ("X's younger siblings were A, B and C"):
  // a sentence/clause break, a new possessive owner, another kin word, a finite
  // verb or a multi-word joiner. Parenthetical life-dates "(born 1844)" are
  // stripped first, so they never stop the walk; bare "and"/commas keep it going.
  function isKinListBoundary(g) {
    var s = String(g).replace(/\([^)]*\)/g, '');
    if (s.length > 90) return true;
    if (/[.!?;]/.test(s)) return true;
    if (/['\u2019]s\b/.test(s)) return true;
    if (/\b(?:who|which|whose|that|when|while|where|because|although|though|unless|until|if|after|before|since|named|called|known|described|portrayed|depicted|renamed|became|born|died|killed|assassinated|executed|was|were|is|are|has|had|married|wed|divorced|raised|reared|adopted|succeeded|preceded|founded|established|built|created)\b/i.test(s)) return true;
    if (/\b(?:as\s+well\s+as|along\s+with|together\s+with|besides|except|rather\s+than|instead\s+of)\b/i.test(s)) return true;
    if (/\b(?:father|mother|son|daughter|brother|sister|sibling|child|children|grandfather|grandmother|grandson|granddaughter|uncle|aunt|nephew|niece|cousin|parent|spouse|wife|husband|widow|widower|partner|fiance|fiancee|boyfriend|girlfriend|rival|friend|colleague|heir|guardian|descendant|protege|student|disciple|mentor|teacher|ward|pupil|step-|half-|great-)\b/i.test(s)) return true;
    return false;
  }

  // Copula- or colon-introduced kin list: "X's younger siblings were A, B and C",
  // "their children: Alistair, Sorcha and Tristan". Every listed name carries the
  // SAME relation to the target. Returns false (and leaves the reading to the
  // ordinary adjacency rules) unless the kin noun is IMMEDIATELY followed by a
  // copula verb / colon AND the span to the first resolvable name is a clean list.
  function emitListKin(targetId, pEnd, rel, txt, ms, sent) {
    var head = txt.slice(pEnd, pEnd + 20);
    var isListIntro = /^\s+(?:were|are|was|is)\b/i.test(head) || /^\s*:/.test(head) || /^\s*\([^)]*\)\s*:/.test(txt.slice(pEnd, pEnd + 40));
    if (!isListIntro) return false;
    var y0 = nextMention(ms, pEnd);
    if (!y0 || y0.start >= sent.end) return false;
    // "X's sister was killed by John" — a verb between kin noun and name means the
    // name is NOT the relative; fall back to the normal per-edge rules. The SAME
    // copula that introduced the list ("were/are/was/is") is stripped from this
    // first gap first — "siblings were James" is the list itself, not a clause.
    var firstGap = txt.slice(pEnd, y0.start).replace(/^\s+(?:were|are|was|is|be)\b(?:\s+the)?/i, '');
    if (isKinListBoundary(firstGap)) return false;
    ensureEdge(y0.id, targetId, rel);
    var at = y0.end - 1;
    for (var li = 0; li < 10; li++) {
      var nx = nextMention(ms, at);
      if (!nx || nx.start >= sent.end) break;
      if (isKinListBoundary(txt.slice(at, nx.start))) break;
      if (nx.id === targetId) break;
      ensureEdge(nx.id, targetId, rel);
      at = nx.end - 1;
    }
    return true;
  }
...
if (x && possGap) {
      ...
      if (y && y.start < sent.end) {
        ...
        // "...Ambedkar persuaded her husband to ask Mahatma Gandhi": an infinitive
        // in the gap means the name that follows is the object of that verb, not
        // the possessor's kin — mirrors the same guard in the main Pattern-1 loop.
        if (/\bto\s+[a-z]/i.test(yGap)) return;
        // Copula- / colon-introduced kin LIST — "Mandell's younger siblings were
        // James (b. 1844), Mary (born and died in 1845) and Mary Ellen (known as
        // Polly)"; "their children: Alistair, Sorcha and Tristan". Every listed
        // name carries the SAME relation; commas, parens and life-dates between
        // items are fine (the "raised N children:" form stays with Pattern 3b).
        if (emitListKin(x.id, pEnd, rel, txt, ms, sent)) return;
        ...
        // F1 inverted appositive: "…by Samprati, Kunala's son" — the relative
        // sits in a bare ", Name" appositive BEFORE the possessive owner and
        // nothing follows the kin noun (the "X's son, Y" read already handled
        // above). Link the preceding appositive mention to the owner.
        var x0 = prevMention(ms, x.start);
        if (x0 && x0.start >= sent.start && x.start - x0.end <= 12) {
          var g0 = txt.slice(x0.end, x.start);
          if (/^,\s*$/.test(g0)) ensureEdge(x0.id, x.id, rel);
        }
      }
      return;
    }
    // Copula- / colon-introduced kin LIST seen from an IMPLICIT owner: "(His|Her
    // |Their|Mandell's) younger siblings were James, Mary and Mary Ellen" when the
    // possessor is not itself a resolvable mention. The owning person-topic is the
    // kin of every listed name; the possessor must match the topic's own name — a
    // pronoun, a "he/she had N <kin>" opener, or a first/middle/last-name match.
    if (!x && owner && isPersonId(owner)) {
      var fragL = txt.slice(sent.start, pStart).split(/[,;(]+/).pop().replace(/\s+/g, ' ').trim();
      var matchOwner = false;
      var posM = /^([A-Z][A-Za-z'-]*(?:\s+[A-Z][A-Za-z'-]*)*)['\u2019]s$/.exec(fragL);
      if (/^(his|her|their|our)$/i.test(fragL)) matchOwner = true;
      else if (/^(?:he|she|they)\s+(?:had|has)\s+((?:(?:an?|the|two|three|four|five|six|seven|eight|nine|ten|several|multiple|[0-9]{1,2})\s+)?)(?:(?:younger|elder|older|eldest|youngest)\s+)?$/i.test(fragL)) matchOwner = true;
      else if (posM) {
        var posC2 = canonName(posM[1]);
        var ownC2 = canonName((NODE_BY_ID[owner] && NODE_BY_ID[owner].name) || '');
        if (posC2 && ownC2 && posC2.length >= 3 && (ownC2.indexOf(posC2) !== -1 || posC2.indexOf(ownC2) !== -1)) matchOwner = true;
      }
      if (matchOwner && emitListKin(owner, pEnd, rel, txt, ms, sent)) return;
    }
    ...
    if (yGap2.trim().length > 24 || HAS_BOUND.test(yGap2)) {
      if (!isEpithetGap(yGap2)) return;
    }
    var subject = null;
    if (x) {
  ...
  function handleQuestion(it) {
    ...
    while ((m = ofByRe.exec(txt))) {
      ...
      for (var o of objs) {
        if (process.env.KINDBG) console.log('[kin-ofBy]', phrase, 'txt', JSON.stringify(txt.slice(pStart, Math.min(txt.length, pStart + 80))), 'subj', JSON.stringify(subj && txt.slice(subj.start, subj.end)), 'poss', poss, 'distP', distPersons(ms, sent), 'a', a, 'objs', objs.map(function (obm) { return txt.slice(obm.start, obm.end); }));
    ...
    while ((m2 = verbRe.exec(txt))) {
      ...
      } else if (isSpouseVerb && owner && isPersonId(owner)) {
        // Pronoun-headed clause ("He married Suhasini Chattopadhyay, the sister of
        // Sarojini Naidu", subject inferred from the question's owning topic) - the
        // same owner fallback Pattern 1 uses for "She ...". Only stand when the
        // clause lead is a bare personal pronoun with no other resolved name.
        spvSent = sentenceAt(txt, m2.index);
        var spvSeg = txt.slice(spvSent.start, m2.index);
        if (m2.index - spvSent.start <= 60 && /\b(?:he|she|his|her|they|their)\b/i.test(spvSeg)) {
          a2 = owner;
          subj = { id: owner, start: spvSent.start, end: spvSent.start };
        }
      }
      ...
      if (kinVerb) {
        ...
        if (isSpouseVerb) {
          ...
          // ... nor when the resolved object is directly the POSSESSOR of a kin
          // noun ("married Börte's niece Öki" — Börte is the aunt, not the one
          // being married; the real spouse "Öki" is unresolvable in this clause,
          // but the anchor is not). Only a possessive straight off the object
          // counts: "married X, Y's daughter" (comma) keeps X as the spouse and
          // treats Y as X's parent.
          var objBridge = txt.slice(obj.end, Math.min(txt.length, obj.end + 40)).replace(/\s+/g, ' ');
          if (/^['\u2019]s\s+(?:(?:great(?:\s*-\s*)?){0,2})?(?:son|daughter|wife|husband|widow|widower|father|mother|child|children|brother|sister|grandson|granddaughter|grandchild|grandchildren|uncle|aunt|nephew|niece|cousin|sibling|partner)\b/i.test(objBridge)) continue;
...
function main() {
  ...
  // Auto-provision kin-mentioned people (Börte, Temüjin, Samprati, Kunala, ...) as
  // person nodes so lineage edges reach minor relatives, not just the curated spine.
  var kinStats = provisionKinPersons(eachQuestion, nodes, topicMap);
  var kinCats = {};
  for (var kinN of nodes) if (kinN.kin === true) kinCats[kinN.type] = (kinCats[kinN.type] || 0) + 1;
  console.log('kin promotion: provisioned ' + kinStats.provisioned + ' persons, promoted ' + kinStats.promoted + ' to person (' + JSON.stringify(kinCats) + ' kin-tagged nodes)');
...
module.exports = { extractRelations: extractRelations, canon: canon, provisionKinPersons: provisionKinPersons };
}