...
function fetchJSON(url, retries) {
  ...
  return new Promise(function(resolve, reject) {
    var req = https.get(url + '&origin=*', { agent: AGENT, headers: { 'User-Agent': 'HealthBot/1.0' } }, function(res) {
      var d = '';
...
async function fetchHealthCategories(existingKeys, newQuestions, seqObj) {
  console.error('--- Health Bodies (category discovery) ---');
  var cats = ['Hospitals_in_India', 'Medical_colleges_in_India', 'Government_hospitals_in_India'];
  var KEYS = ['Founded', 'Beds', 'Number of beds', 'Hospital type', 'Established', 'Number of employees'];
  var count = 0;
  ...
  console.error('  ' + count + ' category-expanded health questions added\n');
}
...
async function main() {
  ...
  await delay(800);
  await fetchHealthCategories(existingKeys, newQuestions, seqObj);
